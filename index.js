...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer being reported (Discord ID)').setStyle(TextInputStyle.Short).setPlaceholder('Discord user ID').setRequired(true).setMinLength(17).setMaxLength(20)),
...
async function handleDepartmentHours(interaction, mode) {
  ...
  if (mode === 'total') {
    ...
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle('Department Hours').addFields(
      { name: 'Department', value: `${department} — ${deptName(department)}` },
      { name: 'Time frame', value: timeframeLabel(timeframe), inline: true },
  ...
  return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(mode === 'leaderboard' ? 'Department Leaderboard' : 'Department Hours').setDescription(description).setFooter({ text: `${department} • ${timeframeLabel(timeframe)}` })] });
}
...
async function handleCommand(interaction) {
  ...
  if (name === 'ridealong-permissions') {
    ...
    if (action === 'view') return interaction.editReply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}` });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
  ...
  if (name === 'anonreport') {
    ...
    const updated = { ...reportRows, department, dateOfIncident: dateOfIncident || null, gameId: gameId || null, clip: clip || null, description: description || null, context: context || null, anonymous: true, reporterId: null };
    setReportRecord(interaction.channel.id, updated);
    await interaction.channel.permissionOverwrites.edit(interaction.user.id, { ViewChannel: false, SendMessages: false }).catch(() => {});
    ...
    await interaction.channel.send({
      ...
      embeds: [buildReportEmbed({ anonymous: true, type: updated.ticketType || 'officer', department, dateOfIncident, gameId, reportedUserId: updated.reportedUserId, clip, description, context })],
      components: [buttons]
    });
  ...
  if (name === 'deptofhours') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(`Top ${department} Officers`).setDescription(description)] });
  }
  ...
  if (name === 'tophours') {
    ...
    const description = rows.length ? rows.map((r, i) => `${i + 1}. <@${r.discordId}> — ${hoursText(Number(r.seconds))}`).join('\n') : 'No recorded hours.';
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle('Top Hours').setDescription(description)] });
  }
  ...
  if (name === 'inactive_officers' || name === 'dept_officers') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xe0a458).setTitle(`${department} Inactive Officers`).setDescription(description).setFooter({ text: `${weeks}+ weeks without duty` })] });
  }
  ...
  if (name === 'promotions') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(`${department} Promotion Eligibility`).setDescription(description).setFooter({ text: `Minimum ${minimum}h this week` })] });
  }
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      if (!department) return interaction.reply({ content: 'Department must be USM, SASP, BCSO or LSPD.', ephemeral: true });
      const officerId = interaction.fields.getTextInputValue('officer').trim();
      if (!/^\d{17,20}$/.test(officerId)) return interaction.editReply({ content: 'Officer field must contain a valid Discord user ID.' });
