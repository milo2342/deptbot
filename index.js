...
async function ensureDepartmentPurchaseCompatibility() {
  const columns = await q(
    `SELECT COLUMN_NAME
     FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'department_purchase_tickets'`
  );
  if (!columns.length) return;
  const names = new Set(columns.map(column => String(column.COLUMN_NAME)));
  if (!names.has('documentsReady')) {
    console.log('Migrating department_purchase_tickets: adding department documentation readiness field.');
    await rawQuery('ALTER TABLE `department_purchase_tickets` ADD COLUMN `documentsReady` TEXT NULL AFTER `discordReady`');
  }
}
...
async function ensureSchema() {
  ...
  await ensureDepartmentPurchaseCompatibility();
...
function departmentPurchaseStepTwoModal() {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('documents_ready').setLabel('Are All Department Documents Ready?').setStyle(TextInputStyle.Short).setPlaceholder('Yes / No - confirm SOPs, policies, training documents, rosters, and required department materials are prepared').setRequired(true).setMaxLength(500)),
      new ActionRowBuilder().addComponents(new TextInputBuilder()
...
function buildDepartmentPurchaseEmbed(application) {
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Department Documentation Readiness', value: clamp(application.documentsReady), inline: false },
  ...
  try {
    await q(
      `INSERT INTO department_purchase_tickets
       (channelId, applicantId, departmentName, reason, jurisdiction, assetsReady, highCommandReady, discordReady, documentsReady, whyAllow, createdAt)
       VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
      [channelId, application.userId, application.departmentName, application.reason, application.jurisdiction,
        application.assetsReady, application.highCommandReady, application.discordReady, application.documentsReady, application.whyAllow, application.createdAt]
    );
  } catch (error) {
    if (!(error instanceof DatabaseUnavailableError)) throw error;
  }
...
async function handleDepartmentPurchaseClose(interaction) {
  ...
  await sendDepartmentPurchaseLog(interaction.guild, closedEmbed, []);
...
async function handleDepartmentPurchaseDelete(interaction) {
  ...
  const deleteEmbed = new EmbedBuilder()
    ...
    .setFooter({ text: `WCRP Department Utilities | ${formatShort(now())}` });
  await sendDepartmentPurchaseLog(interaction.guild, deleteEmbed, []);
...
async function handleClose(interaction) {
  ...
  await interaction.channel.send({ embeds: [ticketClosedEmbed(report, interaction.user.id)], allowedMentions: { parse: [] } });
  ...
  return respond(interaction, { content: 'Ticket closed, logged, and the delete control was added.' });
}
...
async function handleDepartmentPurchaseStepTwo(interaction) {
  ...
  const application = {
    ...
    documentsReady: interaction.fields.getTextInputValue('documents_ready'),
