...
function reportFieldsModal(type, anonymous = false) {
  return new ModalBuilder()
    .setCustomId(`report_modal:${type}:${anonymous ? 'anon' : 'named'}`)
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer being reported (user ID)').setStyle(TextInputStyle.Short).setPlaceholder('Discord user ID, optional').setRequired(false)),
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      if (!requireLogServer(interaction)) return;
      if (!interaction.deferred && !interaction.replied) await interaction.deferReply({ ephemeral: true });
      const [, type, mode] = interaction.customId.split(':');
      const anonymous = mode === 'anon';
      const department = assertDepartment(interaction.fields.getTextInputValue('department'));
      if (!department) return respond(interaction, { content: 'Department must be USM, SASP, BCSO or LSPD.' });
