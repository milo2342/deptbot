...
async function q(sql, params = [], timeoutMs = 4500) {
  const work = pool.execute(sql, params).then(([rows]) => rows);
  const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Database query timed out. Check MYSQL_HOST, MYSQL_PORT and firewall access from Railway.')), timeoutMs));
...
function requireLogServer(interaction, ephemeral = true) {
  if (!requireGuild(interaction)) {
    if (interaction.deferred || interaction.replied) respond(interaction, { content: 'This command can only be used in a server.', ephemeral });
    else interaction.reply({ content: 'This command can only be used in a server.', ephemeral }).catch(() => {});
    return false;
  }
  if (interaction.guild.id !== LOG_GUILD_ID) {
    if (interaction.deferred || interaction.replied) respond(interaction, { content: 'This command is only available in the configured reports and ride-along server.', ephemeral });
...
async function requireAdmin(interaction) {
  if (!requireGuild(interaction)) {
    await respond(interaction, { content: 'This command can only be used in a server.', ephemeral: true });
    return false;
  }
...
async function requireCommandPermission(interaction) {
  ...
  if (!(await commandAllowed(interaction.member, name))) {
    await respond(interaction, { content: 'You do not have permission to use this command.', ephemeral: true });
    return false;
  }
  return true;
}
...
async function totalDutySeconds({ discordId, department, window }) {
  ...
  if (department) { sql += ' AND UPPER(department)=?'; params.push(department); }
  if (window) {
...
async function pollDuty() {
  try {
    const active = await q(`SELECT * FROM duty_hours WHERE (outTime IS NULL OR outTime = 0) AND discordId IS NOT NULL ORDER BY id DESC`);
    const seen = new Set();
    ...
    const completed = await q(`SELECT * FROM duty_hours WHERE outTime IS NOT NULL AND outTime > 0 AND outTime >= ? ORDER BY outTime ASC`, [now() - 30]);
...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer_id').setLabel('Officer Discord ID (optional)').setStyle(TextInputStyle.Short).setRequired(false)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('date').setLabel('Date of incident (optional)').setStyle(TextInputStyle.Short).setRequired(false)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('clip').setLabel('Clip URL (optional)').setStyle(TextInputStyle.Paragraph).setRequired(false)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('details').setLabel('Description / context (optional)').setStyle(TextInputStyle.Paragraph).setRequired(false))
    );
}
...
async function handleDepartmentHours(interaction, mode) {
  ...
  const rows = await q(
    `SELECT discordId,
     ...
     FROM duty_hours
     WHERE UPPER(department)=? AND inTime IS NOT NULL AND inTime < ? AND COALESCE(outTime, UNIX_TIMESTAMP()) > ? AND discordId IS NOT NULL
...
async function handleCommand(interaction) {
  ...
  if (name === 'reportadd') {
    ...
    if (!department) return respond(interaction, { content: 'Department must be USM, SASP, BCSO or LSPD.', ephemeral: true });
    ...
    }).then(channel => respond(interaction, { content: channel ? `Report created: ${channel}` : 'Unable to create the report.', ephemeral: true }));
  }
  ...
  if (name === 'anonreport') {
    ...
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) {
      return createReportTicket({ interaction, type: 'officer', department, anonymous: true, dateOfIncident, gameId, clip, description, context })
        .then(channel => respond(interaction, { content: channel ? `Anonymous report created: ${channel}` : 'Unable to create the anonymous report.', ephemeral: true }));
    }
    const existing = reportRows[0];
    const reportedUserId = existing.reportedUserId || null;
    await q(
    ...
    // Anonymous conversion: remove reporter access, keep only the reported officer (when set) and report staff.
    // Remove every member-specific overwrite from the original ticket except the officer being reported.
    for (const overwrite of interaction.channel.permissionOverwrites.cache.values()) {
      if (overwrite.type === 1 && overwrite.id !== reportedUserId) {
        await interaction.channel.permissionOverwrites.delete(overwrite.id).catch(() => {});
      }
    }
    ...
    await interaction.channel.send({
      ...
      embeds: [buildReportEmbed({ anonymous: true, type: existing.ticketType || 'officer', department, dateOfIncident, gameId, reportedUserId, clip, description, context })],
      components: [buttons]
    });
  ...
  if (name === 'deptofhours') {
    ...
    const rows = await q(
      `SELECT discordId, SUM(GREATEST(0, LEAST(COALESCE(outTime, UNIX_TIMESTAMP()), ?) - GREATEST(inTime, ?))) seconds
       FROM duty_hours WHERE UPPER(department)=? AND inTime < ? AND COALESCE(outTime, UNIX_TIMESTAMP()) > ? AND discordId IS NOT NULL
  ...
  if (name === 'inactive_officers' || name === 'dept_officers') {
    ...
    const rows = await q(`SELECT discordId, MAX(COALESCE(outTime, UNIX_TIMESTAMP())) lastDuty FROM duty_hours WHERE UPPER(department)=? AND discordId IS NOT NULL GROUP BY discordId HAVING lastDuty < ? ORDER BY lastDuty ASC`, [department, cutoff]);
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      if (!department) return interaction.editReply({ content: 'Department must be USM, SASP, BCSO or LSPD.' });
      const officerIdRaw = interaction.fields.getTextInputValue('officer_id')?.trim() || '';
      const officerId = /^\d{17,20}$/.test(officerIdRaw) ? officerIdRaw : null;
