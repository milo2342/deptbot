...
const DB_CONFIGURED = Boolean(MYSQL_HOST && MYSQL_USER && MYSQL_DATABASE);
...
if (!DB_CONFIGURED) console.warn('MYSQL is not fully configured. Discord commands will still start, but database-backed commands will return an error until MYSQL_HOST, MYSQL_USER and MYSQL_DATABASE are set.');
...
async function q(sql, params = []) {
  if (!DB_CONFIGURED) throw new Error('MySQL is not configured.');
  const timeoutMs = 3500;
  let timer;
  try {
    return await Promise.race([
      pool.execute(sql, params),
      new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('MySQL query timed out.')), timeoutMs); })
    ]).then(([rows]) => rows);
  } finally {
    clearTimeout(timer);
  }
...
function requireGuild(interaction, ephemeral = true) {
  if (!interaction.guild) {
    interaction.editReply({ content: 'This command can only be used in a server.', ephemeral });
    return false;
  }
  return true;
}
...
function requireLogServer(interaction, ephemeral = true) {
  ...
  if (interaction.guild.id !== LOG_GUILD_ID) {
    interaction.editReply({ content: 'This command is only available in the configured reports and ride-along server.', ephemeral });
    return false;
  }
  return true;
}
...
async function requireAdmin(interaction) {
  ...
  await interaction.editReply({ content: 'Administrator permission required.', ephemeral: true });
  return false;
}
...
async function createReportTicket({ interaction, type, department, anonymous, dateOfIncident, gameId, clip, description, context, reportedUserId = null }) {
  ...
  if (anonymous && reportedUserId) permissionOverwrites.push({ id: String(reportedUserId), allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] });
...
async function handleDepartmentHours(interaction, mode) {
  ...
  if (mode === 'total') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle('Department Hours').addFields(
      { name: 'Department', value: `${department} — ${deptName(department)}` },
      { name: 'Time frame', value: timeframeLabel(timeframe), inline: true },
  ...
  return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(mode === 'leaderboard' ? 'Department Leaderboard' : 'Department Hours').setDescription(description).setFooter({ text: `${department} • ${timeframeLabel(timeframe)}` })] });
}
...
async function handleCommand(interaction) {
  ...
  if (LOG_SERVER_COMMANDS.has(name) && interaction.guildId !== LOG_GUILD_ID) {
    return interaction.editReply({ content: 'This command is only available in the configured reports and ride-along server.', ephemeral: true });
  }
  ...
  if (name === 'officer-report-panel') {
    ...
    return interaction.editReply({ content: 'Report panel posted.', ephemeral: true });
  }
  ...
  if (name === 'admin-roles') {
    ...
    return interaction.editReply({ content: `Administrator roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'permissions') {
    ...
    if (action === 'view') return interaction.editReply({ content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting(`cmdperm:${command}`, roles, interaction.guild.id);
    return interaction.editReply({ content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}`, ephemeral: true });
  }
  ...
  if (name === 'report-config') {
    ...
    return interaction.editReply({ content: await reportConfig(interaction.guild.id), ephemeral: true });
  }
  ...
  if (name === 'report-staff') {
    ...
    if (action === 'view') return interaction.editReply({ content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('reportStaffRoles', roles, interaction.guild.id);
    return interaction.editReply({ content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'ridealong-permissions') {
    ...
    if (action === 'view') return interaction.editReply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('ridealongRoles', roles, interaction.guild.id);
    return interaction.editReply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'log-config') {
    ...
    return interaction.editReply({ content: `${type}: ${value ? `<#${value}>` : 'Not configured'}`, ephemeral: true });
  }

  if (name === 'addofficer') {
    if (!await ensureReportPermissions(interaction.member)) return interaction.editReply({ content: 'Reports team permission required.', ephemeral: true });
    const user = interaction.options.getUser('user');
    const userId = interaction.options.getString('user_id')?.trim();
    const targetId = user?.id || userId;
    if (!/^\d{17,20}$/.test(String(targetId || ''))) return interaction.editReply({ content: 'Provide a valid Discord user or Discord user ID.', ephemeral: true });
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) return interaction.editReply({ content: 'This is not a report ticket.', ephemeral: true });
  ...
  if (name === 'reportadd') {
    ...
    }).then(channel => interaction.editReply({ content: channel ? `Report created: ${channel}` : 'Unable to create the report.', ephemeral: true }));
  }
  ...
  if (name === 'anonreport') {
    ...
    if (!reportRows[0]) {
      return createReportTicket({ interaction, type: 'officer', department, anonymous: true, dateOfIncident, gameId, clip, description, context })
        .then(channel => interaction.editReply({ content: channel ? `Anonymous report created: ${channel}` : 'Unable to create the anonymous report.', ephemeral: true }));
    }
    ...
    // Anonymous mode removes the original reporter from the ticket. Only the
    // reported person (when known) and configured reports team retain access.
    await interaction.channel.permissionOverwrites.set([
      { id: interaction.guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] },
      ...(reportedUserId ? [{ id: reportedUserId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }] : []),
      ...[...new Set([...roles, ...staffRoles])].filter(id => interaction.guild.roles.cache.has(id)).map(id => ({ id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }))
    ]).catch(() => {});
    ...
    await interaction.channel.send({
      ...
      embeds: [buildReportEmbed({ anonymous: true, type: reportRows[0].ticketType || 'officer', department, dateOfIncident, gameId, reportedUserId: null, clip, description, context })],
      components: [buttons]
    });
  ...
  if (name === 'rename') {
    ...
    if (!rows[0]) return interaction.editReply({ content: 'This is not a report ticket.', ephemeral: true });
  ...
  if (name === 'delete') {
    if (!await ensureReportPermissions(interaction.member) && !(await isAdmin(interaction.member))) return interaction.editReply({ content: 'Reports team permission required.', ephemeral: true });
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) return interaction.editReply({ content: 'This is not a report ticket.', ephemeral: true });
  ...
  if (name === 'ridealong') {
    if (!await roleAllowed(interaction.member, 'ridealongRoles')) return interaction.editReply({ content: 'You do not have permission to log ride-alongs.', ephemeral: true });
  ...
  if (name === 'add_org') {
    ...
    return interaction.editReply({ content: `Added organisation ${code} — ${orgName}.`, ephemeral: true });
  }
  ...
  if (name === 'add_org_hours') {
    ...
    return interaction.editReply({ content: `Added ${hours.toFixed(2)} hours to ${code}.`, ephemeral: true });
  }
  ...
  if (name === 'rename_org') {
    ...
    return interaction.editReply({ content: `Renamed ${oldCode} to ${newCode}.`, ephemeral: true });
  }
}

client.on('interactionCreate', async interaction => {
  try {
    // Acknowledge every slow interaction immediately. Discord only gives an
    // interaction a few seconds to be acknowledged; database/report work can
    // take longer. Command/button/modal handlers below use editReply after this.
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      if (!department) return interaction.editReply({ content: 'Department must be USM, SASP, BCSO or LSPD.', ephemeral: true });
      ...
      return interaction.editReply({ content: channel ? `Report created: ${channel}` : 'Unable to create the report.', ephemeral: true });
    }
    ...
    if (interaction.replied) {
      await interaction.followUp({ content: 'An error occurred while processing that request.' }).catch(() => {});
    } else if (interaction.deferred) {
      await interaction.editReply({ content: 'An error occurred while processing that request.' }).catch(() => {});
    } else {
      await interaction.reply({ content: 'An error occurred while processing that request.' }).catch(() => {});
    }
  }
});
...
client.once('clientReady', async () => {
  ...
  const startDutyTracking = async () => {
    if (!DB_CONFIGURED) {
      console.warn('DATABASE / DUTY TRACKING OFFLINE: MySQL variables are not configured.');
      return;
    }
    try {
      await ensureSchema();
