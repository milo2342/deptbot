...
const DATABASE_TIMEOUT_MS = Math.max(750, Number(process.env.DATABASE_TIMEOUT_MS || 1000));
...
async function q(sql, params = []) {
  ...
  let timer;
  try {
    const work = pool.execute(sql, params);
    const timeout = new Promise((_, reject) => {
      timer = setTimeout(() => reject(new Error(`Database operation timed out after ${DATABASE_TIMEOUT_MS}ms`)), DATABASE_TIMEOUT_MS);
    });
...
async function getSetting(key, fallback = null, guildId = null) {
  const keyName = settingKey(guildId, key);
  if (runtimeSettings.has(keyName)) return runtimeSettings.get(keyName);
  try {
    const rows = await q('SELECT settingValue FROM bot_settings WHERE settingKey=? LIMIT 1', [keyName]);
    const value = rows[0] ? rows[0].settingValue : fallback;
    runtimeSettings.set(keyName, value);
    return value;
  } catch {
    return fallback;
  }
...
async function setSetting(key, value, guildId = null) {
  ...
  runtimeSettings.set(keyName, stringValue);
  q(
    'INSERT INTO bot_settings (settingKey, settingValue) VALUES (?, ?) ON DUPLICATE KEY UPDATE settingValue=VALUES(settingValue)',
    [keyName, stringValue]
  ).catch(error => console.warn(`Setting ${keyName} kept in memory; database unavailable: ${error.message}`));
}
...
function requireLogServer(interaction, ephemeral = true) {
  ...
  if (interaction.guild.id !== LOG_GUILD_ID) {
    interaction.reply({ content: `This command is only available in the reports and ride-along server (${LOG_GUILD_ID}).`, ephemeral });
    return false;
  }
  return true;
}
...
async function handleCommand(interaction) {
  ...
  if (LOG_SERVER_COMMANDS.has(name) && interaction.guildId !== LOG_GUILD_ID) {
    return interaction.reply({ content: `This command is only available in the reports and ride-along server (${LOG_GUILD_ID}).`, ephemeral: true });
  }
  ...
  if (name === 'reportadd') {
    ...
    }).then(channel => interaction.editReply({ content: channel ? `Report created: ${channel}` : 'Unable to create the report.' }));
  }
  ...
  if (name === 'anonreport') {
    ...
    if (!reportRows[0]) {
      return createReportTicket({ interaction, type: 'officer', department, anonymous: true, dateOfIncident, gameId, clip, description, context })
        .then(channel => interaction.editReply({ content: channel ? `Anonymous report created: ${channel}` : 'Unable to create the anonymous report.' }));
    }
    await q(
    ...
    if (existing.reportedUserId) {
      await interaction.channel.permissionOverwrites.edit(existing.reportedUserId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }).catch(() => {});
    }
    ...
    await interaction.channel.send({
      ...
      embeds: [buildReportEmbed({ anonymous: true, type: existing.ticketType || 'officer', department, dateOfIncident, gameId, reportedUserId: existing.reportedUserId || null, clip, description, context })],
      components: [buttons]
    });
