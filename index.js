...
const LOG_GUILD_ID = process.env.LOG_GUILD_ID || '1320847906252525568';
...
const REPORT_STORE_PATH = path.join(__dirname, 'reports-store.json');
let reportStore = {};
try { reportStore = JSON.parse(fs.readFileSync(REPORT_STORE_PATH, 'utf8')); } catch { reportStore = {}; }
function saveReportStore() { try { fs.writeFileSync(REPORT_STORE_PATH, JSON.stringify(reportStore, null, 2)); } catch (e) { console.warn('Unable to save report store:', e.message); } }
const SETTINGS_STORE_PATH = path.join(__dirname, 'settings-store.json');
let settingsStore = {};
try { settingsStore = JSON.parse(fs.readFileSync(SETTINGS_STORE_PATH, 'utf8')); } catch { settingsStore = {}; }
function saveSettingsStore() { try { fs.writeFileSync(SETTINGS_STORE_PATH, JSON.stringify(settingsStore, null, 2)); } catch (e) { console.warn('Unable to save settings store:', e.message); } }
function localSettingKey(guildId, key) { return settingKey(guildId, key); }
...
async function getSetting(key, fallback = null, guildId = null) {
  const sk = settingKey(guildId, key);
  if (Object.prototype.hasOwnProperty.call(settingsStore, sk)) return settingsStore[sk];
  try {
    const rows = await q('SELECT settingValue FROM bot_settings WHERE settingKey=? LIMIT 1', [sk]);
    if (rows[0]) {
      settingsStore[sk] = rows[0].settingValue;
      saveSettingsStore();
...
async function setSetting(key, value, guildId = null) {
  const sk = settingKey(guildId, key);
  settingsStore[sk] = String(value);
  saveSettingsStore();
  q('INSERT INTO bot_settings (settingKey, settingValue) VALUES (?, ?) ON DUPLICATE KEY UPDATE settingValue=VALUES(settingValue)', [sk, String(value)]).catch(() => {});
}
async function getJsonSetting(key, fallback, guildId = null) {
  try { return JSON.parse(await getSetting(key, JSON.stringify(fallback), guildId)); } catch { return fallback; }
}
...
function requireGuild(interaction, ephemeral = true) {
  if (!interaction.guild) {
    (interaction.deferred || interaction.replied ? interaction.editReply({ content: 'This command can only be used in a server.' }) : interaction.reply({ content: 'This command can only be used in a server.', ephemeral }));
    return false;
  }
  return true;
}
...
function requireLogServer(interaction, ephemeral = true) {
  ...
  if (interaction.guild.id !== LOG_GUILD_ID) {
    if (interaction.deferred || interaction.replied) interaction.editReply({ content: 'This command is only available in the configured reports and ride-along server.' }).catch(() => {});
    else interaction.reply({ content: 'This command is only available in the configured reports and ride-along server.', ephemeral }).catch(() => {});
    return false;
  }
  return true;
}
...
async function requireAdmin(interaction) {
  ...
  await (interaction.deferred || interaction.replied ? interaction.editReply({ content: 'Administrator permission required.' }) : interaction.reply({ content: 'Administrator permission required.', ephemeral: true }));
  return false;
}
...
async function requireCommandPermission(interaction) {
  ...
  if (!(await commandAllowed(interaction.member, name))) {
    await (interaction.deferred || interaction.replied ? interaction.editReply({ content: 'You do not have permission to use this command.' }) : interaction.reply({ content: 'You do not have permission to use this command.', ephemeral: true }));
    return false;
  }
  return true;
}
...
const commands = [
  ...
  baseCommand('officerreporttranscript', 'Admin: set the officer report transcript channel'),
...
commands.find(c => c.name === 'ridealong-permissions')
  ...
  .addChannelOption(o => o.setName('channel').setDescription('Ride-along log channel').setRequired(false).addChannelTypes(ChannelType.GuildText));

commands.find(c => c.name === 'officerreporttranscript')
  .addChannelOption(o => o.setName('channel').setDescription('Officer report transcript channel').setRequired(true).addChannelTypes(ChannelType.GuildText));
...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('department').setLabel('Department').setStyle(TextInputStyle.Short).setPlaceholder('USM / SASP / BCSO / LSPD').setRequired(true).setMaxLength(4)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer being reported (Discord ID)').setStyle(TextInputStyle.Short).setPlaceholder('Discord user ID').setRequired(false).setMinLength(17).setMaxLength(20)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('date').setLabel('Date of incident').setStyle(TextInputStyle.Short).setPlaceholder('DD/MM/YYYY').setRequired(true)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('clip').setLabel('Clip / evidence URL').setStyle(TextInputStyle.Paragraph).setRequired(false)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('context').setLabel('Description / additional context').setStyle(TextInputStyle.Paragraph).setRequired(true))
    );
}

function buildReportEmbed({ anonymous, type, department, dateOfIncident, gameId, reportedUserId, clip, description, context }) {
  return new EmbedBuilder()
    .setColor(type === 'higher' ? 0x3498db : 0x3498db)
...
function reportRecord(channelId) { return reportStore[String(channelId)] || null; }
function setReportRecord(channelId, record) { reportStore[String(channelId)] = record; saveReportStore(); return record; }
function deleteReportRecord(channelId) { delete reportStore[String(channelId)]; saveReportStore(); }
...
async function createReportTicket({ interaction, type, department, anonymous, dateOfIncident, gameId, clip, description, context, reportedUserId = null }) {
  ...
  const departmentRoles = await getJsonSetting(`reportRoles:${department}`, [], guild.id).catch(() => []);
  const staffRoles = await getJsonSetting('reportStaffRoles', [], guild.id).catch(() => []);
  const categoryId = await getSetting('reportCategoryId', null, guild.id).catch(() => null);
  ...
  if (anonymous && reportedUserId) permissionOverwrites.push({ id: reportedUserId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] });
  ...
  const record = setReportRecord(channel.id, {
    channelId: channel.id,
    guildId: guild.id,
    ticketType: type,
    department,
    reporterId: anonymous ? null : interaction.user.id,
  ...
  const buttons = reportActionRow(channel.id, false);
  await channel.send({
...
async function updateReportEmbed(channelId, changes) {
  const report = reportRecord(channelId);
  if (!report) return false;
  const next = { ...report, ...changes };
  setReportRecord(channelId, next);
  ...
  await reportMessage.edit({ embeds: [buildReportEmbed(next)] });
  return true;
}
...
async function handleDepartmentHours(interaction, mode) {
  ...
  if (mode === 'total') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle('Department Hours').addFields(
      { name: 'Department', value: `${department} — ${deptName(department)}` },
      { name: 'Time frame', value: timeframeLabel(timeframe), inline: true },
  ...
  return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle(mode === 'leaderboard' ? 'Department Leaderboard' : 'Department Hours').setDescription(description).setFooter({ text: `${department} • ${timeframeLabel(timeframe)}` })] });
}
...
async function handleEvaluate(interaction) {
  ...
  return interaction.editReply({ embeds: [
    new EmbedBuilder()
      .setColor(total >= requiredSeconds ? 0x3498db : 0x3498db)
...
async function handleCommand(interaction) {
  ...
  if (!await requireCommandPermission(interaction)) return;
  const name = interaction.commandName;

  if (LOG_SERVER_COMMANDS.has(name) && interaction.guildId !== LOG_GUILD_ID) {
    return interaction.editReply({ content: 'This command is only available in the configured reports and ride-along server.' });
  }
  ...
  if (name === 'officer-report-panel') {
    ...
    return interaction.editReply({ content: 'Report panel posted.' });
  }
  ...
  if (name === 'admin-roles') {
    ...
    return interaction.editReply({ content: `Administrator roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}` });
  }
  ...
  if (name === 'permissions') {
    ...
    if (action === 'view') return interaction.editReply({ content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}` });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting(`cmdperm:${command}`, roles, interaction.guild.id);
    return interaction.editReply({ content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}` });
  }
  ...
  if (name === 'report-config') {
    ...
    return interaction.editReply({ content: await reportConfig(interaction.guild.id) });
  }
  ...
  if (name === 'report-staff') {
    ...
    if (action === 'view') return interaction.editReply({ content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}` });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('reportStaffRoles', roles, interaction.guild.id);
    return interaction.editReply({ content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}` });
  }
  ...
  if (name === 'ridealong-permissions') {
    ...
    if (action === 'view') {
      const logId = await getSetting('ridealongLogChannelId', null, interaction.guild.id);
      return interaction.editReply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}\nRide-along log channel: ${logId ? `<#${logId}>` : 'Not configured'}` });
    }
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    if (channel) await setSetting('ridealongLogChannelId', channel.id, interaction.guild.id);
    await setJsonSetting('ridealongRoles', roles, interaction.guild.id);
    const logId = await getSetting('ridealongLogChannelId', null, interaction.guild.id);
    return interaction.editReply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}\nRide-along log channel: ${logId ? `<#${logId}>` : 'Not configured'}` });
  }

  if (name === 'officerreporttranscript') {
    const channel = interaction.options.getChannel('channel', true);
    await setSetting('officerReportTranscriptChannelId', channel.id, interaction.guild.id);
    return interaction.editReply({ content: `Officer report transcripts will be saved to ${channel}.` });
  }
  ...
  if (name === 'ridealong-config') {
    ...
    return interaction.editReply({ content: `Ride-along role: ${currentRide ? `<@&${currentRide}>` : 'Not configured'}\nTrainee role: ${currentTrainee ? `<@&${currentTrainee}>` : 'Not configured'}` });
  }
  ...
  if (name === 'log-config') {
    ...
    return interaction.editReply({ content: `${type}: ${value ? `<#${value}>` : 'Not configured'}` });
  }

  if (name === 'addofficer') {
    if (!await ensureReportPermissions(interaction.member)) return interaction.editReply({ content: 'Reports team permission required.' });
    const user = interaction.options.getUser('user');
    const userId = interaction.options.getString('user_id')?.trim();
    const targetId = user?.id || userId;
    if (!/^\d{17,20}$/.test(String(targetId || ''))) return interaction.editReply({ content: 'Provide a valid Discord user or Discord user ID.' });
    const report = reportRecord(interaction.channel.id);
    if (!report) return interaction.editReply({ content: 'This is not a report ticket.' });
    await updateReportEmbed(interaction.channel.id, { reportedUserId: targetId });
    return interaction.editReply({ content: `Officer being reported set to <@${targetId}>.` });
  }
  ...
  if (name === 'reportadd') {
    ...
    return interaction.editReply({ content: channel ? `Report created: ${channel}` : 'Unable to create the report.' });
  }

  if (name === 'anonreport') {
    const department = assertDepartment(interaction.options.getString('department'));
    const dateOfIncident = interaction.options.getString('date');
    const gameId = interaction.options.getString('game_id');
    const clip = interaction.options.getString('clip');
    const description = interaction.options.getString('description');
    const context = interaction.options.getString('context');
    const reportRows = reportRecord(interaction.channel.id);
    if (!reportRows) {
      const reportedUserId = interaction.options.getUser('officer')?.id || null;
      const channel = await createReportTicket({ interaction, type: 'officer', department, anonymous: true, dateOfIncident, gameId, clip, description, context, reportedUserId });
      return interaction.editReply({ content: channel ? `Anonymous report created: ${channel}` : 'Unable to create the anonymous report.' });
    }
    if (!(await ensureReportPermissions(interaction.member))) return interaction.editReply({ content: 'Only the reports team can convert a report to anonymous.' });
    const updated = {
      ...reportRows,
      department,
      dateOfIncident: dateOfIncident || reportRows.dateOfIncident || null,
      gameId: gameId || reportRows.gameId || null,
      clip: clip || reportRows.clip || null,
      description: description || reportRows.description || null,
      context: context || reportRows.context || null,
      anonymous: true,
      reporterId: null
    };
    setReportRecord(interaction.channel.id, updated);
    if (interaction.user.id) await interaction.channel.permissionOverwrites.edit(interaction.user.id, { ViewChannel: false, SendMessages: false, ReadMessageHistory: false }).catch(() => {});
    if (reportRows.reporterId) await interaction.channel.permissionOverwrites.edit(reportRows.reporterId, { ViewChannel: false, SendMessages: false, ReadMessageHistory: false }).catch(() => {});
    const staffRoles = await getJsonSetting('reportStaffRoles', [], interaction.guild.id);
    const deptRoles = await getJsonSetting(`reportRoles:${department}`, [], interaction.guild.id);
    const keepRoleIds = [...new Set([...staffRoles, ...deptRoles])];
    for (const roleId of keepRoleIds) {
      await interaction.channel.permissionOverwrites.edit(roleId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }).catch(() => {});
    }
    if (updated.reportedUserId) await interaction.channel.permissionOverwrites.edit(updated.reportedUserId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }).catch(() => {});
    await clearChannel(interaction.channel);
    await interaction.channel.setName(`anon-${department.toLowerCase()}`).catch(() => {});
    const roles = deptRoles;
    await interaction.channel.send({
      content: roles.length ? roles.map(id => `<@&${id}>`).join(' ') : undefined,
      embeds: [buildReportEmbed(updated)],
      components: [reportActionRow(interaction.channel.id, false)]
    });
    const reportLogId = await getSetting('reportLogChannelId', null, interaction.guild.id);
    const log = reportLogId ? interaction.guild.channels.cache.get(reportLogId) : null;
    if (log) await log.send({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle('Anonymous Report Updated').setDescription(`Department: ${department} — ${deptName(department)}`).addFields(
      { name: 'Date of incident', value: dateOfIncident || 'Not provided', inline: true },
      { name: 'In-Game ID', value: gameId || 'Not provided', inline: true },
      { name: 'Clip', value: clip || 'Not provided' }
    )] }).catch(() => {});
  ...
  if (name === 'rename') {
    const report = reportRecord(interaction.channel.id);
    if (!report) return interaction.editReply({ content: 'This is not a report ticket.' });
    const owner = report.reporterId || report.reportedUserId || interaction.user.id;
    ...
    return interaction.editReply({ content: `Channel renamed to ${base}-handling.` });
  }
  ...
  if (name === 'close') {
    ...
    const report = reportRecord(interaction.channel.id);
    if (!report) return interaction.editReply({ content: 'This is not a report ticket.' });
    if (report.reporterId) await interaction.channel.permissionOverwrites.edit(report.reporterId, { ViewChannel: false, SendMessages: false }).catch(() => {});
    setReportRecord(interaction.channel.id, { ...report, closedAt: now() });
    return interaction.editReply({ content: 'Ticket closed.' });
  }

  if (name === 'delete') {
    if (!await ensureReportPermissions(interaction.member) && !(await isAdmin(interaction.member))) return interaction.editReply({ content: 'Reports team permission required.' });
    const reportRows = reportRecord(interaction.channel.id);
    if (!reportRows) return interaction.editReply({ content: 'This is not a report ticket.' });
    await sendTranscriptToConfiguredChannel(interaction.channel, reportRows).catch(() => {});
    deleteReportRecord(interaction.channel.id);
    await interaction.editReply({ content: 'Transcript saved. Deleting ticket...' });
    setTimeout(() => interaction.channel.delete().catch(() => {}), 800);
    return;
  }

  if (name === 'ridealong') {
    if (!await roleAllowed(interaction.member, 'ridealongRoles')) return interaction.editReply({ content: 'You do not have permission to log ride-alongs.' });
    ...
    ).catch(e => console.warn('Ride-along database log unavailable:', e.message));

    const logChannelId = await getSetting('ridealongLogChannelId', null, interaction.guild.id);
    const logChannel = logChannelId ? interaction.guild.channels.cache.get(logChannelId) : null;
    const embed = new EmbedBuilder()
      .setColor(result === 'Passed' ? 0x3498db : 0x3498db)
  ...
  if (name === 'deptofhours') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle(`Top ${department} Officers`).setDescription(description)] });
  }
  ...
  if (name === 'tophours') {
    ...
    const description = rows.length ? rows.map((r, i) => `${i + 1}. <@${r.discordId}> — ${hoursText(Number(r.seconds))}`).join('\n') : 'No recorded hours.';
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle('Top Hours').setDescription(description)] });
  }
  ...
  if (name === 'inactive_officers' || name === 'dept_officers') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle(`${department} Inactive Officers`).setDescription(description).setFooter({ text: `${weeks}+ weeks without duty` })] });
  }
  ...
  if (name === 'promotions') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x3498db).setTitle(`${department} Promotion Eligibility`).setDescription(description).setFooter({ text: `Minimum ${minimum}h this week` })] });
  }
  ...
  if (name === 'leomulti') {
    ...
    return interaction.editReply({ content: `LEO hour multiplier started at ${multiplier}x for ${duration} minutes.` });
  }
  ...
  if (name === 'add_org') {
    ...
    return interaction.editReply({ content: `Added organisation ${code} — ${orgName}.` });
  }
  ...
  if (name === 'add_org_hours') {
    ...
    return interaction.editReply({ content: `Added ${hours.toFixed(2)} hours to ${code}.` });
  }
  ...
  if (name === 'rename_org') {
    ...
    return interaction.editReply({ content: `Renamed ${oldCode} to ${newCode}.` });
  }
}


async function sendTranscriptToConfiguredChannel(channel, report) {
  const transcript = await makeTranscript(channel);
  const channelId = await getSetting('officerReportTranscriptChannelId', null, channel.guild.id);
  const transcriptChannel = channelId ? await channel.guild.channels.fetch(channelId).catch(() => null) : null;
  if (!transcriptChannel?.isTextBased()) return false;
  const filename = `${channel.name}-${report?.anonymous ? 'anonymous-' : ''}transcript.txt`;
  const summary = new EmbedBuilder()
    .setColor(0x3498db)
    .setTitle('Officer Report Transcript')
    .addFields(
      { name: 'Ticket', value: `#${channel.name}`, inline: true },
      { name: 'Department', value: String(report?.department || 'Unknown'), inline: true },
      { name: 'Officer Reported', value: report?.reportedUserId ? `<@${report.reportedUserId}>` : 'Not provided', inline: true },
      { name: 'Anonymous', value: report?.anonymous ? 'Yes' : 'No', inline: true }
    )
    .setFooter({ text: `WCRP Department Utilities • ${formatShort(now())}` });
  await transcriptChannel.send({ embeds: [summary], files: [{ attachment: Buffer.from(transcript || 'No messages.', 'utf8'), name: filename }] });
  return true;
}

function reportActionRow(channelId, closed = false) {
  const buttons = [
    new ButtonBuilder().setCustomId(`report_transcript:${channelId}`).setLabel('Transcript').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId(`report_delete:${channelId}`).setLabel('Delete').setStyle(ButtonStyle.Danger)
  ];
  if (!closed) buttons.unshift(new ButtonBuilder().setCustomId(`report_close:${channelId}`).setLabel('Close').setStyle(ButtonStyle.Primary));
  return new ActionRowBuilder().addComponents(buttons);
}

async function closeReportTicket(interaction) {
  const report = reportRecord(interaction.channel.id);
  if (!report) return interaction.reply({ content: 'This is not a report ticket.', ephemeral: true });
  ...
  if (report.reporterId) await interaction.channel.permissionOverwrites.edit(report.reporterId, { ViewChannel: false, SendMessages: false }).catch(() => {});
  const next = { ...report, closedAt: report.closedAt || now() };
  setReportRecord(interaction.channel.id, next);
  const messages = await interaction.channel.messages.fetch({ limit: 50 }).catch(() => null);
  const main = messages ? [...messages.values()].find(m => m.embeds[0]?.title?.includes('Report')) : null;
  if (main) await main.edit({ components: [reportActionRow(interaction.channel.id, true)] }).catch(() => {});
  return interaction.reply({ content: 'Ticket closed. Use Transcript to save a transcript or Delete to remove the ticket.' });
}

async function deleteReportTicket(interaction) {
  const report = reportRecord(interaction.channel.id);
  if (!report) return interaction.reply({ content: 'This is not a report ticket.', ephemeral: true });
  if (!(await ensureReportPermissions(interaction.member))) return interaction.reply({ content: 'Reports team permission required.', ephemeral: true });
  await sendTranscriptToConfiguredChannel(interaction.channel, report).catch(() => {});
  deleteReportRecord(interaction.channel.id);
  await interaction.reply({ content: 'Transcript saved. Deleting ticket...' }).catch(() => {});
  setTimeout(() => interaction.channel.delete().catch(() => {}), 800);
}
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('report_close:')) {
      if (!requireLogServer(interaction)) return;
      return closeReportTicket(interaction);
    }

    if (interaction.isButton() && interaction.customId.startsWith('report_transcript:')) {
      if (!requireLogServer(interaction)) return;
      const report = reportRecord(interaction.channel.id);
      if (!report) return interaction.reply({ content: 'This is not a report ticket.', ephemeral: true });
      if (!(await ensureReportPermissions(interaction.member))) return interaction.reply({ content: 'Reports team permission required.', ephemeral: true });
      const ok = await sendTranscriptToConfiguredChannel(interaction.channel, report).catch(() => false);
      return interaction.reply({ content: ok ? 'Transcript saved to the configured officer report transcript channel.' : 'No officer report transcript channel is configured. Use /officerreporttranscript first.', ephemeral: true });
    }

    if (interaction.isButton() && interaction.customId.startsWith('report_delete:')) {
      if (!requireLogServer(interaction)) return;
      return deleteReportTicket(interaction);
    }
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      if (!department) return interaction.editReply({ content: 'Department must be USM, SASP, BCSO or LSPD.', embeds: [] });
      const officerId = interaction.fields.getTextInputValue('officer').trim();
      if (officerId && !/^\d{17,20}$/.test(officerId)) return interaction.editReply({ content: 'Officer field must contain a valid Discord user ID.' });
