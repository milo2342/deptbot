...
if (process.env.MYSQL_PASSWORD) mysqlConfig.password = process.env.MYSQL_PASSWORD;
...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer Discord ID (optional)').setStyle(TextInputStyle.Short).setRequired(false)),
...
async function createReportTicket({ interaction, type, department, anonymous, dateOfIncident, gameId, clip, description, context, reportedUserId = null }) {
  ...
  if (anonymous && reportedUserId && reportedUserId !== interaction.user.id) {
    permissionOverwrites.push({ id: reportedUserId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.ReadMessageHistory] });
  }
...
async function handleCommand(interaction) {
  ...
  if (name === 'anonreport') {
    ...
    if (reportRows[0].reporterId) await interaction.channel.permissionOverwrites.edit(reportRows[0].reporterId, { ViewChannel: false, SendMessages: false, ReadMessageHistory: false }).catch(() => {});
    if (reportedUserId && reportedUserId !== reportRows[0].reporterId) {
      await interaction.channel.permissionOverwrites.edit(reportedUserId, { ViewChannel: true, ReadMessageHistory: true, SendMessages: false }).catch(() => {});
    }
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      const officerId = officerRaw.match(/\d{17,20}/)?.[0] || null;
