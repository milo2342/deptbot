...
async function q(sql, params = []) {
  if (!databaseReady) throw new Error('DATABASE_OFFLINE');
  const queryPromise = pool.execute(sql, params);
  const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Database query timed out after 5000ms.')), 5000));
  return (await Promise.race([queryPromise, timeout]))[0];
}
...
async function getSetting(key, fallback = null, guildId = null) {
  const k = settingKey(guildId, key);
  if (settingsCache.has(k)) return settingsCache.get(k);
  try {
    const rows = await q('SELECT settingValue FROM bot_settings WHERE settingKey=? LIMIT 1', [k]);
    const value = rows[0] ? rows[0].settingValue : fallback;
    settingsCache.set(k, value);
    return value;
  } catch { return fallback; }
}
async function setSetting(key, value, guildId = null) {
  const k = settingKey(guildId, key);
  settingsCache.set(k, String(value));
  try {
    await q('INSERT INTO bot_settings (settingKey, settingValue) VALUES (?, ?) ON DUPLICATE KEY UPDATE settingValue=VALUES(settingValue)', [k, String(value)]);
    return true;
  } catch (error) {
    console.warn(`Unable to persist setting ${k}: ${error.message}`);
    return false;
  }
}
...
async function ensureSchema() {
  ...
  for (const statement of statements) {
    const queryPromise = pool.query(statement);
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Database connection/schema operation timed out after 5000ms.')), 5000));
...
function requireGuild(interaction, ephemeral = true) {
  if (!interaction.guild) {
    respond(interaction, { content: 'This command can only be used in a server.' });
    return false;
  }
  return true;
}
...
function requireLogServer(interaction, ephemeral = true) {
  ...
  if (interaction.guild.id !== LOG_GUILD_ID) {
    respond(interaction, { content: 'This command is only available in the configured reports and ride-along server.' });
    return false;
  }
  return true;
}
...
async function requireAdmin(interaction) {
  ...
  await respond(interaction, { content: 'Administrator permission required.', ephemeral: true });
  return false;
}
...
async function requireCommandPermission(interaction) {
  ...
  if (!(await commandAllowed(interaction.member, name))) {
    await respond(interaction, { content: 'You do not have permission to use this command.' });
    return false;
  }
  return true;
}
...
commands.find(c => c.name === 'anonreport').addStringOption(o => o.setName('officer_id').setDescription('Officer Discord user ID, optional').setRequired(false));
...
function departmentButtons(prefix, type = null) {
  const rows = [];
  for (let i = 0; i < DEPARTMENTS.length; i += 4) {
    rows.push(new ActionRowBuilder().addComponents(
      ...DEPARTMENTS.slice(i, i + 4).map(d => new ButtonBuilder()
        .setCustomId(type ? `${prefix}:${type}:${d}` : `${prefix}:${d}`)
        .setLabel(d)
...
function reportFieldsModal(type, anonymous = false, department) {
  return new ModalBuilder()
    .setCustomId(`report_modal:${type}:${anonymous ? 'anon' : 'named'}:${department}`)
    .setTitle(type === 'higher' ? 'Higher Up Report' : 'Officer Report')
    .addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer being reported').setStyle(TextInputStyle.Short).setPlaceholder('Discord user ID, optional').setRequired(false)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('date').setLabel('Date of incident').setStyle(TextInputStyle.Short).setPlaceholder('DD/MM/YYYY').setRequired(false)),
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('clip').setLabel('Clip URL').setStyle(TextInputStyle.Short).setPlaceholder('Paste clip URL, optional').setRequired(false)),
...
async function createReportTicket({ interaction, type, department, anonymous, dateOfIncident, gameId, clip, description, context, reportedUserId = null }) {
  ...
  if (anonymous && reportedUserId && reportedUserId !== interaction.user.id) {
    permissionOverwrites.push({ id: reportedUserId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] });
  }
...
async function makeTicketAnonymous(interaction, report, reportedUserId) {
  // Anonymous tickets must not remain visible to the original reporter.
  const reporterId = report.reporterId;
  if (reporterId) {
    await interaction.channel.permissionOverwrites.edit(reporterId, { ViewChannel: false, SendMessages: false, ReadMessageHistory: false }).catch(() => {});
  }

  // Keep only the reported officer (when supplied) and configured report staff
  // with access to the ticket. Department report roles are included because
  // they are the department-specific report team/notification roles.
  if (reportedUserId) {
    await interaction.channel.permissionOverwrites.edit(reportedUserId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }).catch(() => {});
  }
  const roles = await getJsonSetting(`reportRoles:${report.department}`, [], interaction.guild.id);
  const staffRoles = await getJsonSetting('reportStaffRoles', [], interaction.guild.id);
  for (const roleId of [...new Set([...roles, ...staffRoles])]) {
...
async function handleDepartmentHours(interaction, mode) {
  ...
  if (mode === 'total') {
    ...
    return respond(interaction, { embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle('Department Hours').addFields(
      { name: 'Department', value: `${department} — ${deptName(department)}` },
      { name: 'Time frame', value: timeframeLabel(timeframe), inline: true },
  ...
  return respond(interaction, { embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(mode === 'leaderboard' ? 'Department Leaderboard' : 'Department Hours').setDescription(description).setFooter({ text: `${department} • ${timeframeLabel(timeframe)}` })] });
}
...
async function handleEvaluate(interaction) {
  ...
  return respond(interaction, { embeds: [
    new EmbedBuilder()
...
async function handleCommand(interaction) {
  ...
  if (LOG_SERVER_COMMANDS.has(name) && interaction.guildId !== LOG_GUILD_ID) {
    return respond(interaction, { content: 'This command is only available in the configured reports and ride-along server.', ephemeral: true });
  }

  if (ADMIN_COMMANDS.has(name) && !(await isAdmin(interaction.member))) {
    return respond(interaction, { content: 'Administrator permission required.', ephemeral: true });
  }

  if (name === 'officer-report-panel') {
    await interaction.channel.send({ embeds: [reportPanelEmbed()], components: [reportPanelRow()] });
    return respond(interaction, { content: 'Report panel posted.', ephemeral: true });
  }
  ...
  if (name === 'admin-roles') {
    ...
    return respond(interaction, { content: `Administrator roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'permissions') {
    ...
    if (action === 'view') return respond(interaction, { content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting(`cmdperm:${command}`, roles, interaction.guild.id);
    return respond(interaction, { content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}`, ephemeral: true });
  }
  ...
  if (name === 'report-config') {
    ...
    return respond(interaction, { content: await reportConfig(interaction.guild.id), ephemeral: true });
  }
  ...
  if (name === 'report-staff') {
    ...
    if (action === 'view') return respond(interaction, { content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('reportStaffRoles', roles, interaction.guild.id);
    return respond(interaction, { content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'ridealong-permissions') {
    ...
    if (action === 'view') return respond(interaction, { content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('ridealongRoles', roles, interaction.guild.id);
    return respond(interaction, { content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'ridealong-config') {
    ...
    return respond(interaction, { content: `Ride-along role: ${currentRide ? `<@&${currentRide}>` : 'Not configured'}\nTrainee role: ${currentTrainee ? `<@&${currentTrainee}>` : 'Not configured'}`, ephemeral: true });
  }
  ...
  if (name === 'log-config') {
    ...
    return respond(interaction, { content: `${type}: ${value ? `<#${value}>` : 'Not configured'}`, ephemeral: true });
  }

  if (name === 'addofficer') {
    if (!await ensureReportPermissions(interaction.member)) return respond(interaction, { content: 'Reports team permission required.', ephemeral: true });
    const user = interaction.options.getUser('user');
    const userId = interaction.options.getString('user_id')?.trim();
    const targetId = user?.id || userId;
    if (!/^\d{17,20}$/.test(String(targetId || ''))) return respond(interaction, { content: 'Provide a valid Discord user or Discord user ID.', ephemeral: true });
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) return respond(interaction, { content: 'This is not a report ticket.', ephemeral: true });
    ...
    return respond(interaction, { content: `Officer being reported set to <@${targetId}>.` });
  }
  ...
  if (name === 'reportadd') {
    ...
    }).then(channel => respond(interaction, { content: channel ? `Report created: ${channel}` : 'Unable to create the report.' }));
  }
  ...
  if (name === 'anonreport') {
    ...
    const anonOfficerId = interaction.options.getString('officer_id')?.trim() || null;
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) {
      return createReportTicket({ interaction, type: 'officer', department, anonymous: true, dateOfIncident, gameId, clip, description, context, reportedUserId: /^\d{17,20}$/.test(String(anonOfficerId || '')) ? anonOfficerId : null })
        .then(channel => interaction.reply({ content: channel ? `Anonymous report created: ${channel}` : 'Unable to create the anonymous report.', ephemeral: true }));
    }
    await interaction.deferReply({ ephemeral: true });
    const reportedUserId = reportRows[0].reportedUserId ? String(reportRows[0].reportedUserId) : null;
    await q(
      `UPDATE reports SET department=?,dateOfIncident=?,gameId=?,clip=?,description=?,context=?,anonymous=1 WHERE channelId=?`,
      [department, dateOfIncident || null, gameId || null, clip || null, description || null, context || null, interaction.channel.id]
    );
    await clearChannel(interaction.channel);
    await interaction.channel.setName(`anon-${department.toLowerCase()}`).catch(() => {});
    await makeTicketAnonymous(interaction, { ...reportRows[0], department }, reportedUserId);
    const roles = await getJsonSetting(`reportRoles:${department}`, [], interaction.guild.id);
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`report_close:${interaction.channel.id}`).setLabel('Close').setStyle(ButtonStyle.Danger));
    await interaction.channel.send({
      content: roles.length ? roles.map(id => `<@&${id}>`).join(' ') : undefined,
      embeds: [buildReportEmbed({ anonymous: true, type: reportRows[0].ticketType || 'officer', department, dateOfIncident, gameId, reportedUserId, clip, description, context })],
      components: [buttons]
    });
    ...
    if (log) await log.send({ embeds: [new EmbedBuilder().setColor(0xb94a48).setTitle('Anonymous Report Updated').setDescription(`Department: ${department} — ${deptName(department)}`).addFields(
      { name: 'Date of incident', value: dateOfIncident || 'Not provided', inline: true },
      { name: 'In-Game ID', value: gameId || 'Not provided', inline: true },
  ...
  if (name === 'rename') {
    ...
    if (!rows[0]) return respond(interaction, { content: 'This is not a report ticket.', ephemeral: true });
  ...
  if (name === 'delete') {
    if (!await ensureReportPermissions(interaction.member) && !(await isAdmin(interaction.member))) return respond(interaction, { content: 'Reports team permission required.', ephemeral: true });
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) return respond(interaction, { content: 'This is not a report ticket.', ephemeral: true });
    ...
    await respond(interaction, { content: 'Saving transcript and deleting ticket...' });
    setTimeout(() => interaction.channel.delete().catch(() => {}), 1000);
    return;
  }

  if (name === 'ridealong') {
    if (!await roleAllowed(interaction.member, 'ridealongRoles')) return respond(interaction, { content: 'You do not have permission to log ride-alongs.', ephemeral: true });
  ...
  if (name === 'deptofhours') {
    ...
    return respond(interaction, { embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(`Top ${department} Officers`).setDescription(description)] });
  }
  ...
  if (name === 'tophours') {
    ...
    const description = rows.length ? rows.map((r, i) => `${i + 1}. <@${r.discordId}> — ${hoursText(Number(r.seconds))}`).join('\n') : 'No recorded hours.';
    return respond(interaction, { embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle('Top Hours').setDescription(description)] });
  }
  ...
  if (name === 'inactive_officers' || name === 'dept_officers') {
    ...
    return respond(interaction, { embeds: [new EmbedBuilder().setColor(0xe0a458).setTitle(`${department} Inactive Officers`).setDescription(description).setFooter({ text: `${weeks}+ weeks without duty` })] });
  }
  ...
  if (name === 'promotions') {
    ...
    return respond(interaction, { embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(`${department} Promotion Eligibility`).setDescription(description).setFooter({ text: `Minimum ${minimum}h this week` })] });
  }
  ...
  if (name === 'leomulti') {
    ...
    return respond(interaction, { content: `LEO hour multiplier started at ${multiplier}x for ${duration} minutes.` });
  }
  ...
  if (name === 'add_org') {
    ...
    return respond(interaction, { content: `Added organisation ${code} — ${orgName}.`, ephemeral: true });
  }
  ...
  if (name === 'add_org_hours') {
    ...
    return respond(interaction, { content: `Added ${hours.toFixed(2)} hours to ${code}.`, ephemeral: true });
  }
  ...
  if (name === 'rename_org') {
    ...
    return respond(interaction, { content: `Renamed ${oldCode} to ${newCode}.`, ephemeral: true });
  }
}

client.on('interactionCreate', async interaction => {
  try {
    // Never leave a user waiting on a database call. Acknowledge immediately.
    if (interaction.isChatInputCommand() && !interaction.deferred && !interaction.replied) {
      await interaction.deferReply({ ephemeral: true });
    }

    // Report type selected from panel -> department selector.
    if (interaction.isStringSelectMenu() && interaction.customId === 'report_type') {
      if (!requireLogServer(interaction)) return;
      await interaction.reply({ content: `Select the department being reported.`, components: departmentButtons('report_department', interaction.values[0]), ephemeral: true });
      return;
    }

    // Department selected -> open the report form with all remaining fields.
    if (interaction.isButton() && interaction.customId.startsWith('report_department:')) {
      if (!requireLogServer(interaction)) return;
      const [, type, deptRaw] = interaction.customId.split(':');
      const department = assertDepartment(deptRaw);
      if (!department) return interaction.reply({ content: 'Invalid department.', ephemeral: true });
      await interaction.showModal(reportFieldsModal(type, false, department));
      return;
    }

    // /anonreport -> department selector, then the same form. If used inside a report ticket,
    // the submit action rebuilds the current ticket and removes the reporter's access.
    if (interaction.isChatInputCommand() && interaction.commandName === 'anonreport') {
      await interaction.editReply({ content: 'Select the department being reported.', components: departmentButtons('anon_department') });
      return;
    }

    if (interaction.isButton() && interaction.customId.startsWith('anon_department:')) {
      if (!requireLogServer(interaction)) return;
      const department = assertDepartment(interaction.customId.split(':')[1]);
      if (!department) return interaction.reply({ content: 'Invalid department.', ephemeral: true });
      await interaction.showModal(reportFieldsModal('officer', true, department));
      return;
    }
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      if (!requireLogServer(interaction)) return;
      const [, type, mode, deptRaw] = interaction.customId.split(':');
      const department = assertDepartment(deptRaw);
      if (!department) return respond(interaction, { content: 'Invalid department.', ephemeral: true });
      const officerRaw = interaction.fields.getTextInputValue('officer')?.trim() || '';
      const reportedUserId = /^\d{17,20}$/.test(officerRaw) ? officerRaw : null;
      const dateOfIncident = interaction.fields.getTextInputValue('date') || '';
      const gameId = interaction.fields.getTextInputValue('game_id') || '';
      const clip = interaction.fields.getTextInputValue('clip') || '';
      const details = interaction.fields.getTextInputValue('details') || '';
      const anonymous = mode === 'anon';

      const current = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]).catch(() => []);
      const existing = current[0];

      if (anonymous && existing) {
        await q(`UPDATE reports SET department=?,dateOfIncident=?,gameId=?,clip=?,description=?,context=?,anonymous=1,reportedUserId=COALESCE(?,reportedUserId) WHERE channelId=?`,
          [department, dateOfIncident || null, gameId || null, clip || null, details || null, null, reportedUserId, interaction.channel.id]);
        const nextRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
        const report = nextRows[0] || { ...existing, department, reportedUserId: reportedUserId || existing.reportedUserId };
        await clearChannel(interaction.channel);
        await interaction.channel.setName(`anon-${department.toLowerCase()}`).catch(() => {});
        await makeTicketAnonymous(interaction, report, report.reportedUserId ? String(report.reportedUserId) : null);
        const roles = await getJsonSetting(`reportRoles:${department}`, [], interaction.guild.id);
        const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`report_close:${interaction.channel.id}`).setLabel('Close').setStyle(ButtonStyle.Danger));
        await interaction.channel.send({
          content: roles.length ? roles.map(id => `<@&${id}>`).join(' ') : undefined,
          embeds: [buildReportEmbed({ anonymous: true, type: report.ticketType || 'officer', department, dateOfIncident, gameId, reportedUserId: report.reportedUserId || null, clip, description: details, context: '' })],
          components: [buttons]
        });
        return respond(interaction, { content: 'The report is now anonymous. The original reporter no longer has access to this ticket.' });
      }

      const channel = await createReportTicket({ interaction, type, department, anonymous, dateOfIncident, gameId, clip, description: details, context: '', reportedUserId });
    ...
    const message = error?.message === 'DATABASE_OFFLINE' ? 'The database is currently unavailable. Discord commands are online, but database-backed actions will not work until the MySQL connection is restored.' : 'An error occurred while processing that request.';
    if (interaction.replied || interaction.deferred) await interaction.editReply({ content: message, components: [] }).catch(() => {});
    else await respond(interaction, { content: message, ephemeral: true }).catch(() => {});
  }
});
