...
if (/^(127\.0\.0\.1|localhost|::1)$/i.test(MYSQL_HOST)) {
  console.warn(`MYSQL_HOST is set to ${MYSQL_HOST}. Railway cannot use your FiveM machine's localhost; use the public MySQL server IP/hostname instead.`);
}
...
function requireGuild(interaction, ephemeral = true) {
  if (!interaction.guild) {
    interaction.reply({ content: 'This command can only be used in a server.', ephemeral });
    return false;
  }
  return true;
}
...
function requireLogServer(interaction, ephemeral = true) {
  ...
  if (interaction.guild.id !== LOG_GUILD_ID) {
    interaction.reply({ content: 'This command is only available in the configured reports and ride-along server.', ephemeral });
    return false;
  }
  return true;
}
...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer being reported (Discord ID)').setStyle(TextInputStyle.Short).setPlaceholder('Optional Discord user ID').setRequired(false)),
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('clip').setLabel('Clip URL(s)').setStyle(TextInputStyle.Paragraph).setRequired(false)),
...
async function handleCommand(interaction) {
  ...
  if (LOG_SERVER_COMMANDS.has(name) && interaction.guildId !== LOG_GUILD_ID) {
    return interaction.reply({ content: 'This command is only available in the configured reports and ride-along server.', ephemeral: true });
  }
  ...
  if (name === 'admin-roles') {
    ...
    return interaction.reply({ content: `Administrator roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'permissions') {
    ...
    if (action === 'view') return interaction.reply({ content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting(`cmdperm:${command}`, roles, interaction.guild.id);
    return interaction.reply({ content: `${command}: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'Everyone'}`, ephemeral: true });
  }
  ...
  if (name === 'report-config') {
    ...
    return interaction.reply({ content: await reportConfig(interaction.guild.id), ephemeral: true });
  }
  ...
  if (name === 'report-staff') {
    ...
    if (action === 'view') return interaction.reply({ content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('reportStaffRoles', roles, interaction.guild.id);
    return interaction.reply({ content: `Report staff roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'ridealong-permissions') {
    ...
    if (action === 'view') return interaction.reply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
    if (action === 'add' && role) roles = [...new Set([...roles, role.id])];
    if (action === 'remove' && role) roles = roles.filter(id => id !== role.id);
    if (action === 'clear') roles = [];
    await setJsonSetting('ridealongRoles', roles, interaction.guild.id);
    return interaction.reply({ content: `Ride-along permission roles: ${roles.length ? roles.map(id => `<@&${id}>`).join(', ') : 'None'}`, ephemeral: true });
  }
  ...
  if (name === 'ridealong-config') {
    ...
    return interaction.reply({ content: `Ride-along role: ${currentRide ? `<@&${currentRide}>` : 'Not configured'}\nTrainee role: ${currentTrainee ? `<@&${currentTrainee}>` : 'Not configured'}`, ephemeral: true });
  }
  ...
  if (name === 'log-config') {
    ...
    return interaction.reply({ content: `${type}: ${value ? `<#${value}>` : 'Not configured'}`, ephemeral: true });
  }
  ...
  if (name === 'addofficer') {
    ...
    if (!/^\d{17,20}$/.test(String(targetId || ''))) return interaction.reply({ content: 'Provide a valid Discord user or Discord user ID.', ephemeral: true });
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) return interaction.reply({ content: 'This is not a report ticket.', ephemeral: true });
    ...
    return interaction.reply({ content: `Officer being reported set to <@${targetId}>.` });
  }
  ...
  if (name === 'reportadd') {
    ...
    }).then(channel => interaction.reply({ content: channel ? `Report created: ${channel}` : 'Unable to create the report.', ephemeral: true }));
  }
  ...
  if (name === 'anonreport') {
    ...
    const reportedUserId = interaction.options.getUser('officer')?.id || reportRows[0].reportedUserId || null;
    await q(
    ...
    // Anonymous mode removes the reporter from the ticket. Keep only the reported officer
    // (when known), the department report roles, and configured report staff roles.
    await clearChannel(interaction.channel);
    if (reportRows[0].reporterId) {
      await interaction.channel.permissionOverwrites.edit(reportRows[0].reporterId, { ViewChannel: false, SendMessages: false }).catch(() => {});
    }
    if (reportedUserId && interaction.guild.members.cache.has(reportedUserId)) {
  ...
  if (name === 'rename') {
    ...
    if (!rows[0]) return interaction.reply({ content: 'This is not a report ticket.', ephemeral: true });
  ...
  if (name === 'delete') {
    if (!await ensureReportPermissions(interaction.member) && !(await isAdmin(interaction.member))) return interaction.reply({ content: 'Reports team permission required.', ephemeral: true });
    const reportRows = await q('SELECT * FROM reports WHERE channelId=? LIMIT 1', [interaction.channel.id]);
    if (!reportRows[0]) return interaction.reply({ content: 'This is not a report ticket.', ephemeral: true });
    ...
    await interaction.reply({ content: 'Saving transcript and deleting ticket...' });
  ...
  if (name === 'deptofhours') {
    ...
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(`Top ${department} Officers`).setDescription(description)] });
  }
  ...
  if (name === 'tophours') {
    ...
    const description = rows.length ? rows.map((r, i) => `${i + 1}. <@${r.discordId}> — ${hoursText(Number(r.seconds))}`).join('\n') : 'No recorded hours.';
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle('Top Hours').setDescription(description)] });
  }
  ...
  if (name === 'inactive_officers' || name === 'dept_officers') {
    ...
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xe0a458).setTitle(`${department} Inactive Officers`).setDescription(description).setFooter({ text: `${weeks}+ weeks without duty` })] });
  }
  ...
  if (name === 'promotions') {
    ...
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x3b82f6).setTitle(`${department} Promotion Eligibility`).setDescription(description).setFooter({ text: `Minimum ${minimum}h this week` })] });
  }
  ...
  if (name === 'leomulti') {
    ...
    return interaction.reply({ content: `LEO hour multiplier started at ${multiplier}x for ${duration} minutes.` });
  }
  ...
  if (name === 'add_org') {
    ...
    return interaction.reply({ content: `Added organisation ${code} — ${orgName}.`, ephemeral: true });
  }
  ...
  if (name === 'add_org_hours') {
    ...
    return interaction.reply({ content: `Added ${hours.toFixed(2)} hours to ${code}.`, ephemeral: true });
  }
  ...
  if (name === 'rename_org') {
    ...
    return interaction.reply({ content: `Renamed ${oldCode} to ${newCode}.`, ephemeral: true });
  }
}

client.on('interactionCreate', async interaction => {
  try {
    // Acknowledge slash commands/buttons/modals immediately so Discord never shows
    // 'The application did not respond' while the database or ticket work is running.
    if (interaction.isChatInputCommand() || interaction.isButton() || interaction.isModalSubmit()) {
      const originalReply = interaction.reply.bind(interaction);
      interaction.reply = (options) => interaction.deferred ? interaction.editReply(options) : originalReply(options);
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      const reportedUserId = officerRaw && /^\d{17,20}$/.test(officerRaw) ? officerRaw : null;
      ...
      return interaction.reply({ content: channel ? `Report created: ${channel}` : 'Unable to create the report.', ephemeral: true });
    }
...
client.once('clientReady', async () => {
  ...
  console.log(`MySQL target: ${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DATABASE}`);
