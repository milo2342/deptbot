    ...
    new StringSelectMenuBuilder()
      ...
      .addOptions(...DEPARTMENTS.map(department =>
        new StringSelectMenuOptionBuilder()
          .setLabel(`${department} - ${deptName(department)}`)
          .setValue(department)
      ))
  );
}
...
function departmentPurchasePanelEmbed() {
  return new EmbedBuilder()
    .setColor(0x5865f2)
    .setTitle('Purchase a Department')
    .setDescription('Use the button below to submit a request to open a new WCRP department. Every question in the application is required.')
...
function departmentPurchaseStepOneModal() {
  return new ModalBuilder()
    ...
    .setTitle('Purchase Department - Step 1')
    .addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('department_name').setLabel('Department you want to open').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(100)),
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('reason').setLabel('Reason for opening').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(1000)),
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('jurisdiction').setLabel('Local, Federal, or State?').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(20)),
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('assets_ready').setLabel('EUP and vehicles already ready?').setStyle(TextInputStyle.Short).setPlaceholder('Yes / No - explain briefly').setRequired(true).setMaxLength(300)),
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('high_command_ready').setLabel('Entire high command team ready?').setStyle(TextInputStyle.Short).setPlaceholder('Yes / No - explain briefly').setRequired(true).setMaxLength(300))
    );
}
...
function departmentPurchaseStepTwoModal() {
  return new ModalBuilder()
    ...
    .setTitle('Purchase Department - Step 2')
    .addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('discord_ready').setLabel('Do you have a Discord ready?').setStyle(TextInputStyle.Short).setPlaceholder('Yes / No - explain briefly').setRequired(true).setMaxLength(300)),
      new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId('why_allow').setLabel('Why should WCRP allow this department?').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(2000))
    );
}
...
function buildDepartmentPurchaseEmbed(application) {
  return new EmbedBuilder()
    .setColor(0x5865f2)
    .setTitle('Department Purchase Request')
    .addFields(
      { name: 'Applicant', value: `<@${application.userId}>`, inline: false },
      { name: 'Department you want to open', value: clamp(application.departmentName), inline: false },
      { name: 'Reason for opening', value: clamp(application.reason), inline: false },
      { name: 'Local, Federal, or State', value: clamp(application.jurisdiction), inline: false },
      { name: 'EUP and vehicles already ready?', value: clamp(application.assetsReady), inline: false },
      { name: 'Entire high command team ready?', value: clamp(application.highCommandReady), inline: false },
      { name: 'Discord ready?', value: clamp(application.discordReady), inline: false },
      { name: 'Why should WCRP allow this department?', value: clamp(application.whyAllow), inline: false }
    )
    .setFooter({ text: `Submitted ${formatShort(application.createdAt || now())}` });
}
...
async function departmentPurchaseConfigText(guildId) {
  ...
  return [
    ...
    `Ticket Category: ${categoryId ? `<#${categoryId}>` : 'Not configured'}`,
    `Log Channel: ${logId ? `<#${logId}>` : 'Not configured'}`
  ].join('\n');
}
...
async function createDepartmentPurchaseTicket(interaction, application) {
  ...
  const channel = await guild.channels.create({
    ...
    type: ChannelType.GuildText,
    parent,
    topic: `WCRP_DEPARTMENT_PURCHASE|applicant=${application.userId}`,
  ...
  if (logChannel?.isTextBased()) {
    await logChannel.send({
      content: validRoles.length ? validRoles.map(id => `<@&${id}>`).join(' ') : undefined,
      embeds: [buildDepartmentPurchaseEmbed(application).addFields({ name: 'Ticket', value: `${channel} (${channel.id})`, inline: false })],
      allowedMentions: { parse: [], roles: validRoles, users: [] }
    }).catch(() => {});
  }
  return channel;
