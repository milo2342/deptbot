...
async function q(sql, params = []) {
  const timeoutMs = Math.max(1000, Number(process.env.MYSQL_QUERY_TIMEOUT_MS || 2500));
  let timer;
  try {
    const result = await Promise.race([
      pool.execute(sql, params),
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(Object.assign(new Error('MySQL query timed out'), { code: 'ETIMEDOUT' })), timeoutMs);
      })
    ]);
    return result[0];
  } catch (error) {
    const message = error?.code ? `${error.code}: ${error.message}` : (error?.message || 'Unknown database error');
...
async function isAdmin(member) {
  ...
  try {
    ...
    console.warn(`Admin-role lookup unavailable: ${error.message}`);
    return false;
  }
}
...
async function commandAllowed(member, commandName) {
  ...
  try {
    ...
    // A database outage must not strand Discord interactions in a pending state.
    // Treat unconfigured command permissions as open until the database is reachable again.
    console.warn(`Permission lookup unavailable for /${commandName}: ${error.message}`);
    return true;
  }
}
...
function requireLogServer(interaction) {
  return Boolean(interaction.guild && interaction.guild.id === LOG_GUILD_ID);
}
...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer being reported (User ID or mention)').setStyle(TextInputStyle.Short).setPlaceholder('@user or Discord user ID').setRequired(false)),
...
async function createReportTicket({ interaction, type, department, anonymous, dateOfIncident, gameId, clip, description, context, reportedUserId = null }) {
  ...
  if (reportedUserId) permissionOverwrites.push({ id: reportedUserId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] });
...
function parseDiscordUserId(value) {
  ...
  return /^[0-9]{17,20}$/.test(raw) ? raw : null;
}
...
async function handleCommand(interaction) {
  ...
  if (!interaction.deferred && !interaction.replied) {
    const privateCommands = new Set([
      ...ADMIN_COMMANDS,
      'close', 'delete', 'rename', 'addofficer', 'reportadd', 'anonreport', 'ridealong',
      'hours'
    ]);
    await interaction.deferReply({ ephemeral: privateCommands.has(name) });
  }
  ...
  if (name === 'addofficer') {
    ...
    const targetId = user?.id || parseDiscordUserId(userId);
    if (!targetId) return respond(interaction, { content: 'Provide a valid Discord user or Discord user ID.', ephemeral: true });
  ...
  if (name === 'anonreport') {
    ...
    const reportedUserId = reportRows[0].reportedUserId || null;
    await q(
      `UPDATE reports SET department=?,dateOfIncident=?,gameId=?,clip=?,description=?,context=?,anonymous=1,reporterId=NULL WHERE channelId=?`,
    ...
    // Anonymous tickets contain only the reported person (when known) and report staff/department roles.
    if (interaction.channel.permissionOverwrites.cache.has(interaction.user.id)) {
...
client.on('interactionCreate', async interaction => {
  try {
    if (interaction.isStringSelectMenu() && interaction.customId === 'report_type') {
      if (!requireLogServer(interaction)) return respond(interaction, { content: 'This panel is only available in the reports and ride-along server.', ephemeral: true });
    ...
    if (interaction.isButton() && interaction.customId.startsWith('report_close:')) {
      if (!requireLogServer(interaction)) return respond(interaction, { content: 'This action is only available in the reports and ride-along server.', ephemeral: true });
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      const channel = await createReportTicket({
        ...
        description: details,
        context: '',
        reportedUserId: parseDiscordUserId(interaction.fields.getTextInputValue('officer'))
      });
      return respond(interaction, { content: channel ? `Report created: ${channel}` : 'Unable to create the report.', ephemeral: true });
    }
    ...
    else await respond(interaction, { content: 'An error occurred while processing that request.', ephemeral: true }).catch(() => {});
  }
});
