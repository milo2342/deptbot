...
async function q(sql, params = []) {
  ...
  queryPromise.catch(() => {});
  const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Database query timed out.')), 2500));
...
function requireGuild(interaction, ephemeral = true) {
  if (!interaction.guild) {
    respond(interaction, { content: 'This command can only be used in a server.', ephemeral });
    return false;
  }
  return true;
}
...
function requireLogServer(interaction, ephemeral = true) {
  ...
  if (interaction.guild.id !== LOG_GUILD_ID) {
    respond(interaction, { content: 'This command is only available in the configured reports and ride-along server.', ephemeral });
    return false;
  }
  return true;
}
...
function reportFieldsModal(type) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('officer').setLabel('Officer reported (user ID)').setStyle(TextInputStyle.Short).setPlaceholder('Optional Discord user ID').setRequired(false)),
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('clip').setLabel('Clips / evidence URL').setStyle(TextInputStyle.Paragraph).setRequired(false)),
...
async function handleCommand(interaction) {
  ...
  if (name === 'anonreport') {
    ...
    const reporterId = reportRows[0].reporterId;
    if (reporterId) await interaction.channel.permissionOverwrites.edit(reporterId, { ViewChannel: false, SendMessages: false }).catch(() => {});
    if (reportedUserId) {
  ...
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('report_modal:')) {
      ...
      const channel = await createReportTicket({
        ...
        description: details,
        context: '',
        reportedUserId: interaction.fields.getTextInputValue('officer')?.trim() || null
      });
