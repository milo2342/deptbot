...
Reports and ride-alongs are hard-locked to server `1499578614298181642`. The optional LOG_GUILD_ID environment variable is not used.
...
The bot acknowledges slash commands immediately before doing MySQL or ticket work, so a slow database cannot make Discord show “Application did not respond”. If MySQL is unavailable, data commands return a database error instead of hanging indefinitely.

The ride-along command is `/ridealong log`. When a ride-along is logged, the configured trainee role is removed from the selected player when present. The configured ride-along role is assigned when the result is Passed.

The officer report panel opens a form after selecting Officer Report or Higher Up Report. The form collects department, officer/user ID, incident date, evidence/clip URL, and description/context, then creates the private ticket automatically. `/anonreport` can be used by the report owner or reports staff; it clears the ticket, removes the reporter's channel access, preserves the reported person, keeps reports staff access, renames the channel to `anon-department`, and reposts the report embed.
