...
- Global slash commands are registered with `Routes.applicationCommands(CLIENT_ID)`.
- Required command options are normalized ahead of optional options before registration.
- Chat input commands are acknowledged immediately with an ephemeral defer, so MySQL latency cannot cause a Discord interaction timeout.
- Database queries have a short timeout and database startup/retry failures do not stop the Discord bot.
- Existing FiveM duty rows using `outTime = 0` are treated as active sessions.
- Department matching is case-insensitive, so FiveM can store `usm`, `USM`, etc.
- Reports and ride-alongs are restricted to Discord server `1499578614298181642`.
- `/anonreport` clears the ticket, removes member-specific access for the original reporter and keeps only the reported officer (when set) plus configured report staff/department roles.
