...
assert.strictEqual(closeSection.includes('getReportHandlerRoles'), false, 'Closing a report ticket must not ping handler roles');
assert.ok(closeSection.includes("sendReportLog(interaction.guild, 'Report Closed'"), 'Close event is not automatically logged');
assert.ok(closeSection.includes('], false);'), 'Closed report log must suppress handler role pings');
...
for (const id of ['department_name', 'reason', 'jurisdiction', 'assets_ready', 'high_command_ready', 'discord_ready', 'documents_ready', 'why_allow']) {
...
assert.strictEqual((step2.match(/new ActionRowBuilder\(\)\.addComponents/g) || []).length, 3, 'Department purchase step 2 should have three rows');
assert.ok(step2.includes("setLabel('Are All Department Documents Ready?')"), 'Professional department documentation readiness question missing');
assert.ok(deptPurchaseSection.includes("{ name: 'Department Documentation Readiness'"), 'Department documentation readiness missing from application embed');
...
const purchaseCloseSection = section('async function handleDepartmentPurchaseClose', 'async function handleDepartmentPurchaseDelete');
assert.ok(purchaseCloseSection.includes('sendDepartmentPurchaseLog(interaction.guild, closedEmbed, [])'), 'Closing department purchase tickets must not ping handler roles');
const purchaseDeleteSection = section('async function handleDepartmentPurchaseDelete', 'async function handleAddDepartment');
assert.ok(purchaseDeleteSection.includes('sendDepartmentPurchaseLog(interaction.guild, deleteEmbed, [])'), 'Deleting department purchase tickets must not ping handler roles');
...
const purchaseCompatibility = section('async function ensureDepartmentPurchaseCompatibility', 'async function ensureSchema');
assert.ok(purchaseCompatibility.includes("TABLE_NAME = 'department_purchase_tickets'"), 'Department purchase table compatibility is not inspected');
assert.ok(purchaseCompatibility.includes("!names.has('documentsReady')"), 'Missing documentsReady field is not detected');
assert.ok(purchaseCompatibility.includes('ADD COLUMN `documentsReady`'), 'Existing department purchase table is not migrated with documentsReady');
...
assert.strictEqual(pkg.version, '1.2.5');
